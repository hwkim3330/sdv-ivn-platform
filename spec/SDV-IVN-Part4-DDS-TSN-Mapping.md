...
## 13. 링 토폴로지와 802.1CB (조사 결과 반영, 2026-09-21)

### 13.1 FRER 이중화는 "2배"가 아니라 "2배 + α"다

802.1CB 로 복제된 스트림은 R-TAG(6바이트) + 가능한 S-TAG(4바이트)만큼 페이로드보다
더 붙는다. 대역 계산은 이미 프레이밍 오버헤드를 전부 세므로(§4) 이 규칙과
일관된다.

### 13.2 백본만 보호하는 것은 거의 의미가 없다

업계 측정(Navet/Pegase 모델, BER 10⁻¹²)에 따르면:

| 보호 범위 | 손실률 개선 |
|---|---|
| 백본 스위치만 이중화, 종단은 단일 경로 | **2~2.5배** |
| 종단(토커·리스너)까지 dual-homing | **약 10⁸~10⁹배** |

**첫 홉과 마지막 홉이 지배적 위험이다.** 스위치 사이 링만 이중화하고 센서·
액추에이터 자체는 단일 연결로 두면, 802.1CB 를 켜는 비용 대비 효과가 거의
없다. REDUNDANT_SAFETY 등급을 쓸 때는 **종단 노드까지 이중 연결**하는 설계를
기본으로 삼는다 — 존 스위치 안쪽만 이중화하는 것은 예산 절감이 아니라 효과
없는 지출이다.

### 13.3 REDUNDANT_SAFETY 는 센서보다 액추에이터·명령에 우선 배정한다

카메라 6대를 그대로 복제하면 24 Gb/s 가 48 Gb/s 로 뛴다. 반면 액추에이터
명령은 원래도 저대역이라 복제 비용이 무시할 만하다. 이 프로파일이
REDUNDANT_SAFETY 를 제어 명령·결함 보고·재구성 신호에 배정하고 센서 원본에는
배정하지 않는 이유가 여기 있다(`profiles/autoware/topic_catalog.yaml` 참조).
카메라 손상은 렌즈 오염 같은 단일 장애점이 이미 있어, 전송 경로를 복제해도
근본 위험이 남는다.

### 13.4 링에서 802.1CB 를 쓸 때 사이클/게이트와 상호작용

TAS 로 보호되는 클래스가 링 위에 있으면, "장애 시 반대 방향으로 도는" 경로가
정상 경로보다 홉 수가 많아진다(N-노드 링에서 최대 N-1 홉). 홉당 지연이 아니라
**홉당 간섭**이 지배적 비용이므로 — 보호되지 않는 클래스가 섞여 있으면 그
간섭이 누적된다 — 긴 우회 경로도 TAS 로 보호돼야 지터 예산이 유지된다. 이
프로파일의 SAFETY_CRITICAL/REDUNDANT_SAFETY 가 전부 TAS 인 이유이기도 하다.

### 13.5 컷스루(cut-through)는 아직 표준화되지 않았다

IEEE 802.1 에 컷스루 전달 표준(P802.1DU)이 초안 단계로 존재하나 확정되지
않았다. 이 플랫폼은 스토어앤포워드를 가정한다. 벤치 장비가 컷스루를 쓴다면,
헤더 손상 프레임이 링을 도는 문제(Specht, IEEE 802.1, 2020)에 대한 답을
장비 쪽에서 받아야 한다 — 이는 `devices/sites/` 현장 파일에 능력으로
기록할 사항이지 이 규격이 가정할 것은 아니다.

출처: IEEE 802.1 공개 문서보관소(ieee802.org/1/files/public), Don Pannell(NXP)
"Practical Use Cases for Ethernet Redundancy" 2020, Johannes Specht "On
Standardization of Cut-Through Forwarding" 2020, ITU-T G.8032, IEC 62439-2/3.
전체 인용은 조사 세션 기록 참조 — 이 절은 요약이며 원 출처의 CONFIRMED/REPORTED
구분을 유지한다.
